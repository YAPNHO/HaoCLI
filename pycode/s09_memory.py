...
def _parse_frontmatter(text: str) -> tuple[dict, str]: # 表示返回两个东西：(dict, str) 即 (meta, body)
    if not text.startswith("---"):
        return {}, text
    parts = text.split("---", 2) # split("分隔符", 最多切几次)，表示最多切两次。
    if len(parts) < 3:
        return {}, text # 说明 Frontmatter 不完整。正常应该有三部分：前面空字符串，YAML，正文
    meta = {}
    for line in parts[1].strip().splitlines(): # strip()：去掉首尾空白。splitlines()：变成列表
        if ":" in line:
            k, v = line.split(":", 1) # split("分隔符", 最多切几次)
            meta[k.strip()] = v.strip().strip('"').strip("'") # .strip('"')：注意：它不是删除所有双引号。如果字符串的开头或结尾是双引号，就把它去掉。
    return meta, parts[2].strip()

# 创建一个 Markdown 文件（带 YAML Frontmatter），然后更新索引。
...
def _rebuild_index(): # _ 开头表示内部使用
    """Rebuild MEMORY.md index from all memory files."""
    lines = [] # 创建一个空列表，准备存放索引的每一行。
    for f in sorted(MEMORY_DIR.glob("*.md")): # 排序，保证索引顺序稳定。
        if f.name == "MEMORY.md":
            continue # 跳过 MEMORY.md
        raw = f.read_text() # 读取 Markdown 文件，raw是整个内容
        meta, body = _parse_frontmatter(raw) # 拆成字典和正文
        name = meta.get("name", f.stem) # 字典的安全读取。"name" 存在则 name = meta["name"]，否则 name = 文件名
        desc = meta.get("description", body.split("\n")[0][:80]) # 如果没有描述，则从正文取，使用换行符切割取索引为0的那一项，限制最多80字符。
...
def extract_memories(messages: list):
    ...
    # 让 LLM 从一段对话中自动提取值得长期保存的 Memory（记忆）。
    prompt = (
        "Extract user preferences, constraints, or project facts from this dialogue.\n"# 用户偏好，限制条件，项目事实
        "Return a JSON array. Each item: {name, type, description, body}.\n"
        "- name: short kebab-case identifier (e.g. 'user-preference-tabs')\n"
        "- type: one of 'user' (user preference), 'feedback' (guidance), "# 用户偏好，行为指导，项目事实，外部引用
