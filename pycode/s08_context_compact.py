...
def _block_type(block): # 单下划线：约定为内部使用，是当前文件内部的工具函数，不是提供给外部使用的公共接口。
    return block.get("type") if isinstance(block, dict) else getattr(block, "type", None) # getattr(对象, 属性名, 属性不存在时的默认值)
...
def _message_has_tool_use(msg):
    ...
    if not isinstance(content, list):
        return False
    return any(_block_type(block) == "tool_use" for block in content) # any(...) 表示只要有一个内容块满足条件，就返回 True。
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        messages[:] = tool_result_budget(messages)    # L3: persist large results first，保留原来的列表对象，只替换里面的所有元素。
