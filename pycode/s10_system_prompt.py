...
s09 的变化：
  - PROMPT_SECTIONS：提示片段的主题键字典
  - assemble_system_prompt(context)：按真实状态选择+连接部分
  - get_system_prompt(context)：通过 json.dumps 确定性缓存
  - agent_loop 使用 get_system_prompt(context) 而不是硬编码的 SYSTEM
...
def get_system_prompt(context: dict) -> str:
    ...
    """
    global _last_context_key, _last_prompt # 在函数内部声明并修改全局变量。
