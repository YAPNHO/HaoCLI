...
DEFAULT_MAX_TOKENS = 8000 # 模型本次回答最多输出 8000 tokens
...
class RecoveryState:
    ...
    def __init__(self):
        self.has_escalated = False # 是否已升级处理，初始为否
        self.recovery_count = 0 # 恢复尝试次数，初始为 0
        self.consecutive_529 = 0 # 连续出现 529 错误的次数
        self.has_attempted_reactive_compact = False # 是否尝试过压缩上下文
        self.current_model = PRIMARY_MODEL # 当前使用的模型，初始为主模型


# 计算等待时间，采用指数退避：重试次数越多，等待越久。
def retry_delay(attempt, retry_after=None):
    """带抖动的指数退避。重试后优先。"""
    if retry_after:
        return retry_after
    base = min(BASE_DELAY_MS * (2 ** attempt), 32000) / 1000
    jitter = random.uniform(0, base * 0.25) # jitter 会加入随机时间，避免大量请求同时再次发送
    return base + jitter


# 调用失败时自动重试
def with_retry(fn, state: RecoveryState): # fn：需要执行的函数。 state：记录错误次数、当前模型等状态
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "ratelimit" in name.lower() or "429" in msg: # 遇到 429，说明请求过于频繁，等待后重试。
            ...
            if "overloaded" in name.lower() or "529" in msg or "overloaded" in msg: # 遇到 529，说明服务器过载，等待后重试。
                state.consecutive_529 += 1
                if state.consecutive_529 >= MAX_CONSECUTIVE_529: # 连续出现 3 次 529，切换到备用模型。
                    if FALLBACK_MODEL:
            ...
            raise # 其他异常通过 raise 原样抛给外层处理。
    raise RuntimeError(f"Max retries ({MAX_RETRIES}) exceeded") #重试 10 次仍然失败，抛出 RuntimeError。


# 判断上下文是否过长
def is_prompt_too_long_error(e: Exception) -> bool:
    """Check whether an API error indicates prompt/context too long."""
    msg = str(e).lower() # 把异常转换成小写字符串，然后检查是否包含这几个短句。它只是根据错误文字进行判断，不负责处理错误。
...
def reactive_compact(messages: list) -> list:
    ...
    tail = messages[-5:] # 只保留最后 5 条消息
    return [{"role": "user",
             "content": "[Reactive compact] Earlier conversation trimmed. "
                        "Continue from where you left off."}, *tail] # *tail 表示把最后 5 条消息展开并放入新列表


# ── Context ──

def update_context(context: dict, messages: list) -> dict:
    """从真实状态导出上下文：存在哪些工具，内存文件是否存在。"""
    memories = ""
...
def agent_loop(messages: list, context: dict):
    """包含 LLM 调用的错误恢复主循环。"""
    system = get_system_prompt(context) # 根据 context 生成系统提示词
    state = RecoveryState() # 创建错误恢复状态
    max_tokens = DEFAULT_MAX_TOKENS

    while True:
        # ── LLM call: with_retry handles 429/529, outer handles rest ──
        try:
            response = with_retry( # lambda 把模型请求包装成一个函数，交给 with_retry() 执行。
        ...
        if response.stop_reason == "max_tokens":
            ...
            if not state.has_escalated:
                max_tokens = ESCALATED_MAX_TOKENS # 把输出上限从 8000 提高到 64000，丢弃刚才不完整的回答，重新请求。
