...
# 注册钩子，这些列表后续用来存放注册进来的回调函数。
HOOKS = {"UserPromptSubmit": [], "PreToolUse": [], "PostToolUse": [], "Stop": []}

def register_hook(event: str, callback):
    HOOKS[event].append(callback)

def trigger_hooks(event: str, *args): # *args 用来传递任意数量的参数给钩函数回调
    for callback in HOOKS[event]: # 遍历指定事件列表中注册的所有回调函数。
        result = callback(*args) # *args参数解包
        if result is not None:  # 说明这个钩子想要干预或中止当前的流程
            return result
    return None
