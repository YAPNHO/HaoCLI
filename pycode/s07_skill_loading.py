...
def _parse_frontmatter(text: str) -> tuple[dict, str]:
    ...
    if not text.startswith("---"):
        return {}, text
    parts = text.split("---", 2) # 最多切割 2 次, 如果格式正确，parts 列表里正好有 3 个元素：["", "YAML 块", "正文"]。
    if len(parts) < 3: # 说明文件只在开头写了 ---，却没有写结尾的 --- 来闭合
        return {}, text
    try:
        meta = yaml.safe_load(parts[1]) or {} # 切出来的第二部分 parts[1] 就是 YAML 配置块。
    except yaml.YAMLError:
        meta = {}
    return meta, parts[2].strip() # 第三部分 parts[2]，也就是 Markdown 正文。  通过 .strip() 去除了前后多余的换行符和空格。
...
def _scan_skills():
    ...
    if not SKILLS_DIR.exists():
        return
    for d in sorted(SKILLS_DIR.iterdir()): # .iterdir() 返回的不是一个普通的列表（List），而是一个迭代器（Iterator）。 Python 会自动**按照路径的完整字符串（即字母顺序）**进行升序排列。
        if not d.is_dir():
            continue
        manifest = d / "SKILL.md"
        if manifest.exists():
            ...
            SKILL_REGISTRY[name] = {"name": name, "description": desc, "content": raw} # raw是完整的md文件
