...
scheduled_jobs: dict[str, CronJob] = {} # 所有任务的仓库
cron_queue: list[CronJob] = [] # 待执行队列，某个任务到时间了会放在这个列表
cron_lock = threading.Lock()
agent_lock = threading.Lock()
_last_fired: dict[str, str] = {}  # job_id → "YYYY-MM-DD HH:MM" 记录每个任务上次触发的时间。


# 判断单个 cron 字段是否匹配某个具体数值
def _cron_field_matches(field: str, value: int) -> bool:
    """将单个 cron 字段与值进行匹配。"""
    if field == "*":
        return True
    if field.startswith("*/"):# 步长匹配
        step = int(field[2:])
        return step > 0 and value % step == 0 # 判断是否是整倍数
    if "," in field: # 列表匹配
    ...
    return value == int(field) # 精确匹配单个数字


def cron_matches(cron_expr: str, dt: datetime) -> bool:
    """检查 5 字段 cron 表达式是否与给定的日期时间匹配。
    标准 cron 语义：当 DOM 和 DOW 都受到约束时，使用 OR。"""
    fields = cron_expr.strip().split() # 去掉首尾空白后按空格切分。
    if len(fields) != 5:
        return False
    minute, hour, dom, month, dow = fields # 依次解包
    dow_val = (dt.weekday() + 1) % 7  # Python Monday=0 → cron Sunday=0，完成映射
    ...
    if not (m and h and month_ok): # “与”的关系，必须全部满足
        return False
    # DOM and DOW: if both constrained, either matching is enough (OR)
    dom_unconstrained = dom == "*"
    dow_unconstrained = dow == "*"
    if dom_unconstrained and dow_unconstrained: # 日期和星期为*，表示每天
        return True
...
def _validate_cron_field(field: str, lo: int, hi: int) -> str | None:
    """验证单个 cron 字段值在 [lo, hi] 范围内。"""
    if field == "*":
        return None
    if field.startswith("*/"):
        step_str = field[2:]
        if not step_str.isdigit(): # .isdigit()：是否只包含数字字符
...
def validate_cron(cron_expr: str) -> str | None:
    """验证 cron 表达式。返回错误消息或无。"""
    fields = cron_expr.strip().split()
    if len(fields) != 5:
        return f"Expected 5 fields, got {len(fields)}"
    bounds = [(0, 59), (0, 23), (1, 31), (1, 12), (0, 6)]
    names = ["minute", "hour", "day-of-month", "month", "day-of-week"]
    for i, (field, (lo, hi), name) in enumerate(zip(fields, bounds, names)): # 将三个列表按索引配对，每次迭代取三个列表的同一位置的元素
...
def load_durable_jobs():
    ...
    try:
        ...
        for j in jobs:
            job = CronJob(**j) # 用字典创建 CronJob 对象
...
def schedule_job(cron: str, prompt: str, recurring: bool = True,
    ...
    if err:
        return err
    job = CronJob(
        id=f"cron_{random.randint(0, 999999):06d}", # 整数补零到 6 位
        cron=cron, prompt=prompt,
        recurring=recurring, durable=durable,
    )
    with cron_lock: # 自动获取和释放锁，等价于 Java 的 synchronized(lock) { ... }
        scheduled_jobs[job.id] = job
    if durable:
        save_durable_jobs()
...
def cron_scheduler_loop():
    """独立守护线程：每 1 秒轮询一次，触发匹配作业。
    捕获单个作业错误以防止出现一项糟糕的作业
    杀死整个调度程序线程。"""
    while True:
        time.sleep(1) # 每秒检查一次，轮询间隔是 1 秒
        now = datetime.now() # 当前精确时间（含秒、毫秒）
        # 日期感知标记可防止日常作业在第 2 天以上跳过
        minute_marker = now.strftime("%Y-%m-%d %H:%M") # 精确到分钟的字符串
        with cron_lock:
            for job in list(scheduled_jobs.values()): # 创建副本。如果直接遍历字典，遍历过程中修改字典会抛出 RuntimeError。list() 先复制一份快照，安全遍历。
                try:
...
def consume_cron_queue() -> list[CronJob]:
    """使用 cron_queue 中触发的作业（由 agent_loop 调用）。"""
    with cron_lock:
...
def agent_loop(messages: list, context: dict) -> dict:
    ...
    while True:
        # Layer 4: consume fired cron jobs → inject as messages 注入到期定时任务
...
# 当Agent空闲时，自动执行已触发的定时任务（cron jobs）。
